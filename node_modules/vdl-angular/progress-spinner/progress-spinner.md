`<vdl-progress-spinner>` and `<vdl-spinner>` are a circular indicators of progress and activity.

<!-- example(progress-spinner-overview) -->

### Progress mode
The progress-spinner supports two modes, "determinate" and "indeterminate". 
The `<vdl-spinner>` component is an alias for `<vdl-progress-spinner mode="indeterminate">`.
