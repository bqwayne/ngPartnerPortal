`VdlSnackBar` is a service for displaying snack-bar notifications.
...
In either case, an `VdlSnackBarRef` is returned. This can be used to dismiss the snack-bar or to 
recieve notification of when the snack-bar is dismissed. For simple messages with an action, the 
`VdlSnackBarRef` exposes an observable for when the action is triggered.
...
### Dismissal
A snack-bar can be dismissed manually by calling the `dismiss` method on the `VdlSnackBarRef` 
