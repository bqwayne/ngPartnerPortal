...
@mixin vdl-progress-bar-theme($theme) {
  $primary: map-get($theme, primary);
  $accent: map-get($theme, accent);
  $warn: map-get($theme, warn);

  .vdl-progress-bar-background {
    background: #{_vdl-progress-bar-buffer($primary, 100)};
  }

  .vdl-progress-bar-buffer {
    background-color: vdl-color($primary, 100);
  }

  .vdl-progress-bar-fill::after {
    background-color: vdl-color($primary, 600);
  }

  .vdl-progress-bar.vdl-accent {
    .vdl-progress-bar-background {
      background: #{_vdl-progress-bar-buffer($accent, 100)};
    }

    .vdl-progress-bar-buffer {
      background-color: vdl-color($accent, 100);
    }
    .vdl-progress-bar-fill::after {
      background-color: vdl-color($accent, 600);
    }
  }

  .vdl-progress-bar.vdl-warn {
    .vdl-progress-bar-background {
      background: #{_vdl-progress-bar-buffer($warn, 100)};
    }

    .vdl-progress-bar-buffer {
      background-color: vdl-color($warn, 100);
    }
    .vdl-progress-bar-fill::after {
      background-color: vdl-color($warn, 600);
    }
  }
}
...
// </svg>
@function _vdl-progress-bar-buffer($palette, $hue) {
  $color: vdl-color($palette, $hue) + '';
